...
def generate_html(vulns, conf):
  ...
  templateLoader = jinja2.FileSystemLoader(searchpath="/root/Desktop/recon_collector/templates")
...
def generate_txt(vulns):
  filename = r'report-{}-{}.txt'.format(utils.generate_uuid(), utils.get_date())
  data = ''
...
def generate_xml(vulns):
  filename = r'report-{}-{}.xml'.format(utils.generate_uuid(), utils.get_date())
